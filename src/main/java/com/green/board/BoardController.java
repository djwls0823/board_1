...
/*
    ...
    대용량의 데이터를 보내야할 때도 body에 데이터를 담아서 보낸다. url은 길이 제한이 있기 때문에
    ...
    이렇게 표현하는 문자열이다.
    {}는 객체를 의미하고 []는 배열을 의미한다. ""문자열, 숫자형은 ""없이 표현한다. Key는 무조건 ""로 감싸줘야 한다.
    ...
    일단 데이터가 있었을 때는 get방식이 처리 속도가 빠름, 데이터 처리가 아닌 단순 화면을 띄울 때도 get방식을 사용함
    ...
    GET, DELETE 방식은 Query String or Path Variable을 사용해서 데이터를 보낸다.
...
public class BoardController {
    ...
    //@RequestBody는 요청이 올 때 데이터가 JSON 형태로 오니까 거기에 맞춰서 데이터를 받자.
